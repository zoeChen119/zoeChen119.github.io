...
队友做的93% F1
* ## Method2：SKEP作为预训练模型
### 1. <font color="LightSeaGreen">用Accuracy作为train过程中的evaluate函数的metric，测试的时候再用F1score</font>
#### 1.1 batch_size=16 Acc=63.85%
其他超参数：
    max_seq_length = 128
    batch_size = 16
    learning_rate = 4e-5
...
#### 1.2 batch_size=64 Acc=71.46%(存档1)
其他超参数：
    max_seq_length = 128
    batch_size = 64
    learning_rate = 4e-5
...
#### 1.3 batch_size=64 loss_weight=[2.31,1.0]
其他超参数(与1.2保持一致)：
    max_seq_length = 128
    batch_size = 64
    learning_rate = 4e-5
...
### 2. <font color="LightSeaGreen">直接用F1score作为train过程中的evaluate函数的metric</font>
